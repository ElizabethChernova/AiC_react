...
function addLeftUsers(main, additional) {
  for (let i = 0; i < additional.length; i += 1) {
    if (!main.includes(additional[i])) {
      main.push(additional[i]);
    }
  }
}
function validateUser(user) {
  const nameCheck = typeof user.full_name === 'string' && user.full_name[0].toUpperCase() === user.full_name[0];
  const noteCheck = typeof user.note === 'string' || user.note == null;
  const cityCheck = (typeof user.city === 'string' && user.city[0].toUpperCase() === user.city[0]) || user.city == null;
   const genderCheck = typeof user.gender === 'string' && user.gender[0].toUpperCase() === user.gender[0] || user.gender == null;
  const stateCheck = (typeof user.state === 'string' && user.state[0].toUpperCase() === user.state[0]) || user.state == null;
 const ageCheck = !Number.isNaN(user.age) || user.age == null;
  return nameCheck && genderCheck && stateCheck && noteCheck && cityCheck && ageCheck;
}
function filterUsers(users, country, age, gender, favorite) {
  let filteredUsers = [...users];
  if (country != null) {
    filteredUsers = filteredUsers.filter((entry) => entry.country === country);
  }
  if (age != null) {
    filteredUsers = filteredUsers.filter((entry) => entry.age == age);
  }
  if (gender != null) {
    filteredUsers = filteredUsers.filter((entry) => entry.gender === gender);
  }
  if (favorite != null) {
    filteredUsers = filteredUsers.filter((entry) => entry.favorite === favorite);
  }
...
function findUserbyAge(users, value, comparator) {
...
console.log('~~~~~~~~~change users to right format~~~~~~~~~');
randomUserMock.forEach((element, index) => {
  randomUserMock[index] = changeFormatData(element);
});
addLeftUsers(randomUserMock, additionalUsers);
randomUserMock.forEach((entry) => {
  console.log(entry);
});
// - task 2-
console.log('~~~~~~~~~~~~~~~~~validate~~~~~~~~~~~~~~~~~~~~');
randomUserMock.forEach((entry) => {
  console.log(validateUser(entry) === true ? 'Valid user: true' : 'Not valid user: false');
});
// -- task 3--
console.log('~~~~~~~~~~~~~~~~~filter~~~~~~~~~~~~~~~~~~~~');
const filteredUsers = filterUsers(randomUserMock, 'Ireland', null, 'male', null);
filteredUsers.forEach((entry) => {
  console.log(entry);
});
// --Task 4-
console.log('~~~~~~~~~~~~~~~~~sort~~~~~~~~~~~~~~~~~~~~');
const sortedUsers = sortUsers(randomUserMock, 'full_name', 'dsc');
sortedUsers.forEach((entry) => {
  console.log(entry);
});
// --Task 5--
console.log('~~~~~~~~~~~~~~~~~found~~~~~~~~~~~~~~~~~~~~');
const foundUsers = searchForUser(randomUserMock, '38');
console.log(foundUsers === undefined ? 'No users found' : foundUsers);
const foundUsers2 = searchForUser(randomUserMock, 'Norbert');
...
//--Lab4 --
lab4_1(randomUserMock);


function lab4_1(array){
var html = '<table border="1" padding = "10" id="container">  <tr><th>Ім`я</th><th>Вік</th><th>Стать</th><th>Країна</th></tr>';
for(var i = 0; i < array.length; i++)
{html += '<td>' + array[i].full_name + '</td>';
...
function SortName(){
 ...
 var a = ( sortUsers(randomUserMock, 'full_name', 'asc'));
 $('#container')[0].innerHTML =lab4_1test(a);

}
function SortReverceName(){
    var a = ( sortUsers(randomUserMock, 'full_name', 'dsc'));
 $('#container')[0].innerHTML =lab4_1test(a);
}
function SortAge(){
    var a = ( sortUsers(randomUserMock, 'age', 'asc'));
 $('#container')[0].innerHTML =lab4_1test(a);
}

function SortReverceAge(){
    var a = ( sortUsers(randomUserMock, 'age', 'dsc'));
    $('#container')[0].innerHTML =lab4_1test(a);
    
}
function SortReverceGender(){
    var a = ( sortUsers(randomUserMock, 'gender', 'dsc'));
    $('#container')[0].innerHTML =lab4_1test(a);
}
function SortGender(){
    var a = ( sortUsers(randomUserMock, 'gender', 'asc'));
    $('#container')[0].innerHTML =lab4_1test(a);
}
function SortReverceCountry(){
    var a = ( sortUsers(randomUserMock, 'country', 'dsc'));
    $('#container')[0].innerHTML =lab4_1test(a);
}
function SortCountry(){
    var a = ( sortUsers(randomUserMock, 'country', 'asc'));
    $('#container')[0].innerHTML =lab4_1test(a);
}
// function FilterName(){
//     //users, country, age, gender, favorite
//     var a = ( filterUsers(randomUserMock, 'full_name', 'asc'));
//     $('#container')[0].innerHTML =lab4_1test(a);
   
//    }

   function FilterAge(){
       var a = ( filterUsers(randomUserMock, null, 38));
    $('#container')[0].innerHTML =lab4_1test(a);
   }
   

   function FilterGender(){
       var a = ( filterUsers(randomUserMock, null,null,"male"));
       $('#container')[0].innerHTML =lab4_1test(a);
   }

   function FilterCountry(){
       var a = ( filterUsers(randomUserMock, "Ireland"));
       $('#container')[0].innerHTML =lab4_1test(a);
   }
   function SearchTeacher(){
    ...
    var a="";
    if(gender =="other"){
         a = ( filterUsers(randomUserMock, null, age,)); 
    }else{
        
     a = ( filterUsers(randomUserMock, null, age,gender));
    }
    var reg = $("#region").val(); 
    console.log(a);
    $('#container')[0].innerHTML =lab4_1test(a);
   ...
   function AddTeacher(){
    randomUserMock.push(returnNewTEacher());
   }
   function returnNewTEacher(){

    
  
 
   return {
        gender:$("#gender_popup").val(),
      
        title: "",
        full_name: `${ $("#name_popup").val()} ${ $("#surname_popup").val()}  ${ $("#mittename_popup").val() }`,
        city: $("#city_popup").val(),
        state: "",
        country: $("#country_popup").val(),
        postcode: "",
        coordinates: "",
        timezone: "",
        email:$("#email_popup").val(),
        b_date: $("#date_popup").val(),
        age: "",
        phone:$("#phoneNumber_popup").val(),
        picture_large: "",
        picture_thumbnail: "",
        id:"",
        favorite: false,
        course: $("#country_popup").val(),
        bg_color:$("#neededColor_popup").val(),
        note: $("#comments_popup").val(),
      };
   }